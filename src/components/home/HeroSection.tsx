      ...
      <div className="container relative z-10 mx-auto px-6 md:px-12 lg:px-16 py-32 md:py-40">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-3xl space-y-8">
          ...
          <motion.h1 className="text-5xl md:text-6xl font-bold leading-[1.1] tracking-tight text-white lg:text-6xl">
            {t("hero.title").split(" ").map((word, index) => <motion.span key={index} variants={wordVariants} className="inline-block mr-3 md:mr-4">
                {word}
              </motion.span>)}
          </motion.h1>

          {/* Subtitle */}
          <motion.p variants={wordVariants} className="text-xl md:text-2xl leading-relaxed max-w-2xl" style={{
          color: '#E0E0E0'
        }}>
            {t("hero.subtitle")}
          </motion.p>
          ...
          <motion.div variants={wordVariants} className="flex flex-col sm:flex-row gap-4">
            <AnimatedButton>
              <Button asChild size="lg" className="h-14 px-8 text-white rounded-full text-base font-semibold w-full sm:w-auto transition-all duration-300 hover:scale-[1.03]" style={{
            ...
            <AnimatedButton>
              <Button asChild size="lg" variant="outline" className="h-14 px-8 bg-white/10 backdrop-blur-sm border-2 border-white/30 text-white hover:bg-white/20 hover:border-white/40 rounded-full text-base font-semibold w-full sm:w-auto transition-all duration-300">
